 ...
 */
const create = async ({ userId, title, message, type = "GENERAL" }) => {
 ...
 */
const createMany = async (userIds, { title, message, type = "GENERAL" }) => {
  ...
  const result = await prisma.notification.createMany({
    data: uniqueIds.map((userId) => ({ userId, title, message, type })),
  });
  ...
  uniqueIds.forEach((userId) => {
    emitToUser(userId, "notification:new", { userId, title, message, type, createdAt });
  });
